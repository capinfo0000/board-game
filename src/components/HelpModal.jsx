...
const CARD_HELP = [
  ...
  ['Q', '+30', '4枚'],
  ['K', '+40', '4枚'],
  ['-10 / -20', '場の合計を下げる（0で止まる）', '各4枚'],
  ['101 ⚡', '場の合計を一気に101にする', '4枚'],
  ['⏭ パス', '合計を変えずに手番を終える', '4枚'],
  ['🚫 スキップ', '次の人を飛ばす', '4枚'],
  ['➕➕ ダブル', '次の人は強制で2枚出す。受けた人が特殊カードを出せば1枚でOK（2枚出す番は次の人へ／ダブルでは返せない）。数字・±・101なら2枚必要。2枚出したターンに引けるのは1枚', '4枚'],
  ['🔄 リバース', '順番を逆にする（2人時は実質スキップ）', '4枚'],
  ['1️⃣ リセット', '場の合計を1に戻す', '4枚'],
  ['🎯 ショット', '次に出す人を指名する（自分以外）', '4枚'],
];
  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <h3>カード一覧（全22種・各4枚＝合計88枚）</h3>
        <div className="help-list">
          {CARD_HELP.map(([k, v, n]) => (
            <div className="help-row" key={k}>
              <span className="k">{k}</span>
              <span>{v}</span>
              <span className="help-count">{n}</span>
            </div>
          ))}
        </div>
